...
def main(archive_path: str, output_dir: str, output_formats: List[str], system_message: str):
    ...
    # 3. Identify threads and conversations
    threads, conversations = ex.extract_threads_and_conversations(all_content)
    ...
    if 'markdown' in output_formats:
        ...
        for thread in threads:
            expt.save_thread_markdown(thread, threads_output_dir, os.path.join(archive_path, 'data', 'tweets_media'), images_folder)
        
        # Save standalone tweets by date
        expt.save_tweets_by_date(all_content, threads, tweets_by_date_dir, images_folder)

    if 'oai' in output_formats:
        # Save as OpenAI JSONL
        oai_output_path = os.path.join(output_dir, 'conversations_oai.jsonl')
        expt.save_conversations_to_jsonl(threads, conversations, oai_output_path, system_message)
